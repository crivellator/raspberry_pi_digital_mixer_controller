...
void istanza_clock()
{
  while(softwareRunning) invio_dati_canale();
}
...
void callback_carica(Fl_Widget* w)
{
  ...
  if (!sfogliaFile.show()) // se l'utente non annulla la selezione file, si procede così
  {
    ...
    file.close();
    // una volta caricato il file scorriamo tutti i bottoni in modo che il mixer scriva i dati di ogni canale
    for (int i = 0; i < NUMERO_BOTTONI_CANALI; i++)
    {
      bottoniBanco[i]->do_callback(bottoniBanco[i], reinterpret_cast<void*>(static_cast<intptr_t>(i)));
      delay(10); // probabilmente non necessario
    }
    bottoniBanco[0]->do_callback(bottoniBanco[0], reinterpret_cast<void*>(static_cast<intptr_t>(0))); // e poi si sposta direttamente al banco 1 (posizione 0)
  }
}
